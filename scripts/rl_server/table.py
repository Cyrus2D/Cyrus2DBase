...
class QTable:
    ...
    def get_best_action_d(self, state_c):
        state_d = self.discretize_state(state_c)
    ...
    def get_random_action_c(self, state_c, patch_number, p_rnd=None):
        best_action_d = self.get_best_action_d(state_c)
        if random.random() < p_rnd:
            best_action_d = random.randint(0, 35)
        return self.continues_action(best_action_d)
    ...
    def update_from_buffer(self):
        ...
        if len(transits) == 0:
            return
        for t in transits:
            d_state = self.discretize_state(t.state)
            d_action = self.discretize_action(t.action)
            max_q_s = 0
            if t.next_state is not None:
                next_state_best_action_d = self.get_best_action_d(t.next_state)
                d_next_state_d = self.discretize_state(t.next_state)
                max_q_s = self.q_table[d_next_state_d][next_state_best_action_d]
            self.q_table[d_state][d_action] += 0.1 * (t.reward + 0.95 * (max_q_s - self.q_table[d_state][d_action]))
