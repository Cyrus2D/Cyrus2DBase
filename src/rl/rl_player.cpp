 ...
 */
int RLPlayer::player_port = 0;

RLPlayer::RLPlayer()
    : PlayerAgent(),
      M_communication()
{
    std::cout<<"RLPlayer"<<std::endl;
 ...
 */
bool
RLPlayer::initImpl( CmdLineParser & cmd_parser )
{
    std::cout<<"initImpl"<<std::endl;
...
RLPlayer::actionImpl()
{
    ...
    std::cout<<"actionImpl2"<<std::endl;
    if ( doPreprocess() )
    {
        std::cout<<"actionImpl3"<<std::endl;
...
RLPlayer::getFieldEvaluator() const
{
...
*/
FieldEvaluator::ConstPtr
RLPlayer::createFieldEvaluator() const
{
...
RLPlayer::createActionGenerator() const
{
